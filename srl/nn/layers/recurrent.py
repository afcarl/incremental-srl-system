...
from core import Unit, sigmoid, tanh
...
class GRU(Unit):
    ...
    def one_step(self, x_t, h_tm1):
        xr = T.dot(x_t, self.W_xr) + self.b_xr
        xz = T.dot(x_t, self.W_xz) + self.b_xz
        xh = T.dot(x_t, self.W_xh) + self.b_xh
        r_t = sigmoid(xr + T.dot(h_tm1, self.W_hr))
        z_t = sigmoid(xz + T.dot(h_tm1, self.W_hz))
        h_hat_t = tanh(xh + T.dot((r_t * h_tm1), self.W_hh))
        h_t = (1. - z_t) * h_tm1 + z_t * h_hat_t
        return h_t
